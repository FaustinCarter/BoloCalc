...
class Calculate:
    def __init__(self, log, exp, corr=True):
        ...
        self.log.log("Calculating sensitivity for experiment %s" % (self.exp.name), 1)

        self.chans = [[[ch for ch in camera.channels.values()] for camera in telescope.cameras.values()] for telescope in self.exp.telescopes.values()]
        self.cams  = [[[cm for i  in cm.channels.values()    ] for cm     in telescope.cameras.values()] for telescope in self.exp.telescopes.values()]
        self.teles = [[[tp for i  in cm.channels.values()    ] for cm     in tp.cameras.values()       ] for tp        in self.exp.telescopes.values()]
